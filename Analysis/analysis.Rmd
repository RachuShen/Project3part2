...
This analysis presents the analysis performed on data `my_penguins` and `my_gapminder`. To begin, we first load the data, function and external package.
...
plot1 <- my_gapminder %>%
  ...
  labs(title = "Actual lifeExp versus fitted lifeExp")
...
In this section, we'll predict `body_mass_g` using covariates `bill_length_mm`, `bill_depth_mm`, and `flipper_length_mm` with `k` in `c(2, 5, 10)`. We'll repeat simulation 30 times for each `k`.
...
```{r}
saveRDS(mse, file = "../Output/Results/mse.RDS")
```
...
tbl <- data.frame(tbl)
write_csv(tbl, file = "../Output/Results/summary.csv")
```
